    ...
    const amrex::Real areaFactor, const TransParm* ltransparm, const amrex::Real dt)
{
    ...
    // 3. Freeze X (mole fractions), H (enthalpies), and Cv at T^n.
    ...
    //    Cv converts De/Dt [J/(kg·s)] to dT/dt [K/s] in the sub-step predictor.
    ...
    for (int i = 0; i < N; i++) {
        ...
        eos.RTY2Cv(density[i], primitives[i][NUM_SPECIES], Y, Cv_arr[i]);
    ...
    for (int sub = 0; sub < N_sub; sub++) {
        ...
        // Advance prim_work: T from De/Dt divided by Cv; Y from DY/Dt directly
        for (int i = 0; i < N; i++) {
            prim_work[i][NUM_SPECIES] += dt_sub * (src_T[i] + src_T_sp[i]) * scale / Cv_arr[i];
            for (int sp = 0; sp < NUM_SPECIES; sp++)
    ...
    // 6. Write averaged rates to rhs in conservative form for CVODE:
    //    d(rho*eint)/dt = density * De/Dt
    //    d(rho*Y_sp)/dt = density * DY_sp/Dt
    const amrex::Real inv_N = 1.0 / static_cast<amrex::Real>(N_sub);
    for (int i = 0; i < N; i++) {
        rhs[i][NUM_SPECIES] += density[i] * src_T_sum[i] * inv_N * scale;
        for (int sp = 0; sp < NUM_SPECIES; sp++)
            rhs[i][sp] += density[i] * src_Y_sum[i][sp] * inv_N * scale;
    }
}
